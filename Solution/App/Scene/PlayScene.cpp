...
void PlayScene::obj3dInit()
{
	...
	// ----------
	{
		bossStartTime = std::make_unique<Timer::timeType>();
	...
	// ----------
	{
		...
		playerBulStartTime = std::make_unique<Timer::timeType>();
...
void PlayScene::updatePlayerBullet()
{
	if (playerBul.second)
	{
...

		{
			...
			const Timer::timeType pBulStartTime = *playerBulStartTime;
			const Timer::timeType pBulNowTime = timer->getNowTime() - pBulStartTime;
			...
			constexpr Timer::timeType bulLife = Timer::oneSec;
...
void PlayScene::updateBoss()
{
	...
	const Timer::timeType nowTime = timer->getNowTime() - *bossStartTime;

	bossPos.x = dxBase->nearSin(nowTime / Timer::oneSecF * XM_PIDIV2) * moveRange;
	bossPos.z = dxBase->nearCos(nowTime / Timer::oneSecF * XM_1DIVPI) * moveRange;
	boss->setPos(bossPos);
}
...
void PlayScene::drawImGui()
{
	...
	if (guiWinAlive)
	{
		...
		ImGui::Text("時間 <- %.6f秒",
					float(timer->getNowTime()) / float(Timer::oneSec));
